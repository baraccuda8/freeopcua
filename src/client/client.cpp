...
namespace OpcUa
{
    ...
    {
        LOG_INFO(Logger, "{:90}| starting", FUNCTION_LINE_NAME);

        while(!StopRequest)
        {
            int64_t t_sleep = Period * 0.7;
            LOG_DEBUG(Logger, "{:90}| sleeping for: {}ms", FUNCTION_LINE_NAME, t_sleep);
            ...
            LOG_DEBUG(Logger, "{:90}| renewing secure channel", FUNCTION_LINE_NAME);
            ...
            LOG_DEBUG(Logger, "{:90}| read a variable from address space to keep session open", FUNCTION_LINE_NAME);
    ...
    void KeepAliveThread::Stop()
    {
        ...
        LOG_DEBUG(Logger, "{:90}| stopping", FUNCTION_LINE_NAME);
        ...
        {
            LOG_ERROR(Logger, "{:90}| exception thrown at attempt to join: {}", FUNCTION_LINE_NAME, ex.what());
    ...
    EndpointDescription UaClient::SelectEndpoint(const std::string& endpoint)
    {
        ...
        LOG_DEBUG(Logger, "{:90}| going through server endpoints and selected one we support", FUNCTION_LINE_NAME);
        ...
        {
            LOG_DEBUG(Logger, "{:90}| examining endpoint: {} with security: {}", FUNCTION_LINE_NAME, ed.EndpointUrl, ed.SecurityPolicyUri);

            if(ed.SecurityPolicyUri == "http://opcfoundation.org/UA/SecurityPolicy#None")
            {
                LOG_DEBUG(Logger, "{:90}| security policy is OK, now looking at user token", FUNCTION_LINE_NAME);

                if(ed.UserIdentityTokens.empty())
                {
                    LOG_DEBUG(Logger, "{:90}| server does not use user token, OK", FUNCTION_LINE_NAME);
                ...
                for(UserTokenPolicy token : ed.UserIdentityTokens)
                {
                    if(has_login)
                    {
                        ...
                        {
                            LOG_DEBUG(Logger, "{:90}| endpoint selected", FUNCTION_LINE_NAME);
        ...
        throw std::runtime_error(FUNCTION_LINE_NAME + "| No supported endpoints found on server");
    }
    ...
    void UaClient::Connect(const EndpointDescription& endpoint)
    {
        ...
        LOG_INFO(Logger, "{:90}| creating session ...", FUNCTION_LINE_NAME);
        ...
        LOG_INFO(Logger, "{:90}| create session OK", FUNCTION_LINE_NAME);

        LOG_INFO(Logger, "{:90}| activating session ...", FUNCTION_LINE_NAME);
        ...
        ActivateSessionParameters sessionParameters;
        {
          //const SessionData &session_data = response.Session;
            ...
            {
                throw std::runtime_error(FUNCTION_LINE_NAME + "| Cannot find suitable user identify token for session");
            }
        }
        ...
        LOG_INFO(Logger, "{:90}| activate session OK", FUNCTION_LINE_NAME);
    ...
    {
        if(!Server) { throw std::runtime_error(FUNCTION_LINE_NAME + "| Not connected"); }
    ...
    uint32_t UaClient::GetNamespaceIndex(std::string uri)
    {
        ...
        throw std::runtime_error(FUNCTION_LINE_NAME + "| Error namespace uri does not exists in server");
        //return -1;
    }
    ...
    void UaClient::DeleteNodes(std::vector<OpcUa::Node>& nodes, bool recursive)
    {
        ...
        LOG_DEBUG(Logger, "{:90}| deleting nodes ...", FUNCTION_LINE_NAME);
...
// server response does not contain information needed to encrypt password
  ...
  // use RSA-OAEP encryption if server certificate and nounce is provided
        LOG_DEBUG(Logger, "{:90}| encrypting password RSA-OAEP");
        ...
        LOG_DEBUG(Logger, "{:90}| loading server certificate ... {}", hex(response.Parameters.ServerCertificate.Data));
        ...
        if(ret != 0)
        {
            LOG_ERROR(Logger, "{:90}| error loading server certificate {}", error2string(ret));
            goto exit1;
        }
        {
            ...
            LOG_DEBUG(Logger, "{:90}| seeding the random number generator...");
            ret = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, (const unsigned char*)pers, sizeof(pers));
            if(ret != 0)
            {
                LOG_ERROR(Logger, "{:90}| error seeding the random number generator {}", error2string(ret));
                goto exit2;
            }
            {
                ...
                LOG_DEBUG(Logger, "{:90}| generating the RSA encrypted value...");
                ...
                if(ret != 0)
                {
                    LOG_ERROR(Logger, "{:90}| error RSA encryption {}", error2string(ret));
                    goto exit2;
                }
                LOG_DEBUG(Logger, "{:90}| encrypted password: {}", hex(std::vector<unsigned char>(buff, buff + sizeof(buff))));
