...
void Subscription::PublishCallback(Services::SharedPtr server, const PublishResult result)
{
  ...
  LOG_DEBUG(Logger, "{:90}| called with {} notifications", FUNCTION_LINE_NAME, result.NotificationMessage.NotificationData.size());
  ...
  for (const NotificationData & data : result.NotificationMessage.NotificationData)
    {
      if (data.Header.TypeId == ExpandedObjectId::DataChangeNotification)
        {
          LOG_DEBUG(Logger, "{:90}| notification is of type DataChange", FUNCTION_LINE_NAME);
        ...
        {
          LOG_DEBUG(Logger, "{:90}| notification is of type Event", FUNCTION_LINE_NAME);
        ...
        {
          LOG_DEBUG(Logger, "{:90}| notification is of type StatusChange", FUNCTION_LINE_NAME);
        ...
        {
          LOG_WARN(Logger, "{:90}| unknown notficiation type received: {}", FUNCTION_LINE_NAME, data.Header.TypeId);
        }
    }
  ...
  for (const MonitoredItems & item : data.DataChange.Notification)
    {
      ...
      if (mapit == AttributeValueMap.end())
        {
          LOG_WARN(Logger, "{:90}| got PublishResult for an unknown monitoreditem id: {}", FUNCTION_LINE_NAME, item.ClientHandle);
        }
      ...
      else
        {
          ...
          LOG_DEBUG(Logger, "{:90}| calling DataChange user callback: {} and node: {}", FUNCTION_LINE_NAME, item.ClientHandle, mapit->second.TargetNode);
  ...
  for (EventFieldList ef : data.Events.Events)
    {
      ...
      if (mapit == AttributeValueMap.end())
        {
          LOG_WARN(Logger, "{:90}| got PublishResult for an unknown MonitoredItem id: {}", FUNCTION_LINE_NAME, ef.ClientHandle);
        }
      ...
      else
        {
          ...
          if (mapit->second.Filter.Event.SelectClauses.size() != ef.EventFields.size())
            {
              throw std::runtime_error(FUNCTION_LINE_NAME + "| receive event format does not match requested filter");
            }
          ...
          LOG_DEBUG(Logger, "{:90}| calling client event callback", FUNCTION_LINE_NAME);

          Client.Event(mapit->second.MonitoredItemId, ev);

          LOG_DEBUG(Logger, "{:90}| callback call finished", FUNCTION_LINE_NAME);
        }
    }
}
...
uint32_t Subscription::SubscribeDataChange(const Node & node, AttributeId attr)
{
  ...
  if (results.size() != 1) { throw std::runtime_error(FUNCTION_LINE_NAME + "| should have returned exactly one result"); }
...
std::vector<uint32_t> Subscription::SubscribeDataChange(const std::vector<ReadValueId> & attributes)
{
  ...
  if (results.size() != attributes.size())
    {
      throw (std::runtime_error(FUNCTION_LINE_NAME + "| server did not send answer for all MonitoredItem requests"));
    }
  ...
  for (const auto & res : results)
    {
      ...
      LOG_DEBUG(Logger, "{:90}| storing monitoreditem with handle: {} and id: {}", FUNCTION_LINE_NAME, itemsParams.ItemsToCreate[i].RequestedParameters.ClientHandle, res.MonitoredItemId);
...
void Subscription::UnSubscribe(std::vector<uint32_t> handles)
{
  ...
  for (auto id : handles)
    {
      LOG_DEBUG(Logger, "{:90}| sending unsubscribe for MonitoredItem id: {}", FUNCTION_LINE_NAME, id);
...
uint32_t Subscription::SubscribeEvents(const Node & node, const Node & eventtype)
{
  ...
  LOG_DEBUG(Logger, "{:90}| subscribing events with filter for:", FUNCTION_LINE_NAME);
...
uint32_t Subscription::SubscribeEvents(const Node & node, const EventFilter & eventfilter)
{
  ...
  if (results.size() != 1)
    {
      throw (std::runtime_error(FUNCTION_LINE_NAME + "| CreateMonitoredItems should return one result"));
    }
